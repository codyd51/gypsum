    ...
    def handle_subframe_emitted(self, satellite_id: GpsSatelliteId, emit_subframe_event: EmitSubframeEvent) -> Sequence[Event]:
        ...
        if False:
            if time_params_for_this_satellite.is_complete():
                ...
                satellite_time_of_week_in_seconds = emit_subframe_event.handover_word.time_of_week_in_seconds
                satellite_time = (time_params_for_this_satellite.week_number * SECONDS_PER_WEEK) + satellite_time_of_week_in_seconds
        ...
        satellites_with_complete_orbital_parameters = {sv_id: op for sv_id, op in self.satellite_ids_to_orbital_parameters.items() if op.is_complete()}
        if len(satellites_with_complete_orbital_parameters) >= 4:
            ...
            for satellite_id, orbital_params in satellites_with_complete_orbital_parameters.items():
                ...
                computed_mean_motion: RadiansPerSecond = math.sqrt((earth_gravitational_constant / (semi_major_axis ** 3)))
                ...
                mean_anomaly_at_reference_time = orbital_params.get_parameter(OrbitalParameterType.MEAN_ANOMALY_AT_REFERENCE_TIME)
                mean_anomaly_now = mean_anomaly_at_reference_time + (corrected_mean_motion * time_to_ephemeris_reference_time)
                ...
                for i in range(10):
                    numerator = (mean_anomaly_now - eccentric_anomaly_now_estimation + (eccentricity * math.sin(eccentric_anomaly_now_estimation)))
        ...
        if False:
            ...
            for gps_timestamp, sv_ids_and_params_tups in gps_timestamps_to_satellites_and_orbital_parameters.items():
                if len(sv_ids_and_params_tups) >= 4:
                    print(f'Found 4 full sets of orbital params for GPS timestamp: {gps_timestamp}: {sv_ids_and_params_tups}')
                else:
